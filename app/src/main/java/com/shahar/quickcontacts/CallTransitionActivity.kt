...
class CallTransitionActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // The launcher stays visible behind us. There is no app-colored page and no dim layer.
        ...
        val name = intent.getStringExtra("name").orEmpty().ifBlank {
            if (demo) "הדגמה" else "איש קשר"
        }
        val number = intent.getStringExtra("number").orEmpty()

        overlay = CallOverlayView(name, demo).also { setContentView(it) }

        if (demo) {
            handler.postDelayed({ finishWithoutTransition() }, 2800)
        } else {
            // Long enough to enjoy the motion, short enough that calling still feels immediate.
            handler.postDelayed({ startCall(number) }, 1150)
        }
    }
    ...
    override fun onBackPressed() = finishWithoutTransition()
    ...
    private inner class CallOverlayView(
        private val contactName: String,
        private val demo: Boolean
    ) : View(this@CallTransitionActivity) {
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            // Three perfectly periodic expanding rings. No random timing or jitter.
            repeat(3) { i ->
                val p = (phase + i / 3f) % 1f
            ...
            // A symmetric 8-dot orbit, moving at one constant speed.
            ...
            // Soft halo + stable center. The center never changes size, so the motion feels anchored.
            ...
            // A tiny, transparent status chip below the animation.
            ...
            canvas.drawRoundRect(
                cx - chipW / 2f, chipY - chipH / 2f,
                cx + chipW / 2f, chipY + chipH / 2f,
                dp(22f), dp(22f), fillPaint
            )
